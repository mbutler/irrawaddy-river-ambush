...
| **FR-040** | **Tactical kill-zone map** | **Implemented:** `river-graphic.png` (1672×941) as painted Irrawaddy channel. Remotion overlays squad tokens, `boat.png` rafts on calibrated `RIVER_PATH`, tracers, HUD. Map area ~840×474 inside tactical panel. Hex ranges remain simulation-only in EAL panel | **PNG** map + optional path calibrator (`remotion/public/calibrate-river-path.html`) |
| **TOKENS** | Unit token library | **Rafts:** `boat.png` sprite (scaled/rotated on river path). **Squads:** SVG/circle tokens on near bank. States: normal / active / sinking (X overlay), troop count badge, LMG dot | PNG boat + SVG squad markers |
